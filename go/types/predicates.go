			...
			if x.NumFields() == y.NumFields() {
				for i, f := range x.Fields {
					...
					if f.Anonymous != g.Anonymous ||
						...
						!isIdenticalInternal(f.Type, g.Type, p) {
						return false
					}
				}
				return true
		...
		if y, ok := y.(*Tuple); ok {
			if len(*x) == len(*y) {
				if x != nil {
					for i, v := range *x {
						w := (*y)[i]
						if !isIdenticalInternal(v.Type, w.Type, p) {
							return false
						}
					}
				}
	...
	case *Signature:
		...
		if y, ok := y.(*Signature); ok {
			return x.IsVariadic == y.IsVariadic &&
				isIdenticalInternal(NewTuple(x.Params...), NewTuple(y.Params...), p) &&
				isIdenticalInternal(NewTuple(x.Results...), NewTuple(y.Results...), p)
		}
	...
	case *Interface:
		...
		if y, ok := y.(*Interface); ok {
			...
			if len(a) == len(b) {
				...
				for i, f := range a {
					g := b[i]
					if f.Id() != g.Id() || !isIdenticalInternal(f.Type, g.Type, q) {
						return false
					}
				}
				return true
