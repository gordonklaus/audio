...
func (check *checker) call(x *operand, e *ast.CallExpr) exprKind {
	...
	default:
		...
		case 1:
			x.mode = value
			x.typ = sig.Results[0].Type // unpack tuple
		default:
			x.mode = value
			x.typ = NewTuple(sig.Results...)
		}
		x.expr = e
...
func (check *checker) argument(sig *Signature, i int, x *operand, passSlice bool) {
	...
	case sig.IsVariadic:
		typ = sig.Params[n-1].Type
		if debug {
			if _, ok := typ.(*Slice); !ok {
				check.dump("%s: expected unnamed slice type, got %s", sig.Params[n-1].Pos(), typ)
			}
		}
	default:
...
func (check *checker) selector(x *operand, e *ast.SelectorExpr) {
	...
	if x.mode == typexpr {
		...
		x.mode = value
		x.typ = &Signature{
			Params:     append([]*Var{NewVar(token.NoPos, check.pkg, "", x.typ)}, params...),
			Results:    sig.Results,
